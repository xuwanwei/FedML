...
from fedml_api.standalone.fed_3.fed3_api import Fed3API
...
def test_truthfulness(device, args, dataset, model_trainer):
    fed_3API = Fed3API(device, args, dataset=dataset, model_trainer=model_trainer)
    truth_ratio_list = []
    utility_list = []
    ...
    file_name = 'fed3-{}-IC-{}'.format(args.seed, datatime)
...
def test_budget_balance_with_client_num(device, args, dataset, model_trainer):
    ...
    for client_num in np.arange(10, 100, 10):
        args.client_num_in_total = client_num
        fed_3API = Fed3API(device, args, dataset, model_trainer)
    ...
    timestamp = time.time()
    datatime = time.strftime("%Y-%m-%d-%H-%M", time.localtime(timestamp))
    file_name = 'fed3-{}-BB-{}'.format(args.seed, datatime)
    print("writing {}".format(file_name))
    with open('{}/{}.csv'.format(DATA_PATH, file_name), mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(truth_data)
...
def test_individual_rationality(device, args, dataset, model_trainer):
    ...
    timestamp = time.time()
    datatime = time.strftime("%Y-%m-%d-%H-%M", time.localtime(timestamp))
    file_name = 'fed3-{}-IR-{}'.format(args.seed, datatime)
    print("writing {}".format(file_name))
    with open('{}/{}.csv'.format(DATA_PATH, file_name), mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(truth_data)
...
def test_with_rounds(dataset, device, args, model_trainer):
    fed3API = Fed3API(device=device, args=args, dataset=dataset, model_trainer=model_trainer)
    acc_list, loss_list, time_list, ti_sum_list, round_list = fed3API.train()
    goal_list = []
    ...
    timestamp = time.time()
    datatime = time.strftime("%Y-%m-%d-%H-%M", time.localtime(timestamp))
    file_name = 'fed3-{}-INFO-{}'.format(args.seed, datatime)
    print("writing {}".format(file_name))
    with open('{}/{}.csv'.format(DATA_PATH, file_name), mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
...
def test_with_budget(dataset, device, args, model_trainer):
    ...
    timestamp = time.time()
    datatime = time.strftime("%Y-%m-%d-%H-%M", time.localtime(timestamp))
    file_name = 'fed3-{}-B-INFO-{}'.format(args.seed, datatime)
    print("writing {}".format(file_name))
    with open('{}/{}.csv'.format(DATA_PATH, file_name), mode="w", encoding="utf-8-sig", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(data_table)
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="fedml",
        name="Fed3-r" + str(args.comm_round) + "-e" + str(args.epochs),
        config=args
    )
